...
namespace HoverRace {
	namespace Client {
		...
		/**
		 * Initializes the CentralisedNetworkSession.  Sets default values; main character
		 * creation will be sent at least 5 seconds before the game starts.
		 */
		CentralisedNetworkSession::CentralisedNetworkSession(BOOL pInternetGame, int pMajorID,
		...
		/**
		 * Destroy the CentralisedNetworkSession.  Clean things up, report the match.
		 */
		CentralisedNetworkSession::~CentralisedNetworkSession()
		{
		 ...
		 */
		int CentralisedNetworkSession::ResultAvaillable() const
		{
			int lReturnValue = 0;
		 ...
		 */
		void CentralisedNetworkSession::GetResult(int pPosition, const char*& pPlayerName, int& pId, BOOL& pConnected, int& pNbLap, MR_SimulationTime& pFinishTime, MR_SimulationTime& pBestLap) const
		{
		 ...
		 */
		void CentralisedNetworkSession::GetHitResult(int pPosition, const char*& pPlayerName, int& pId, BOOL& pConnected, int& pNbHitOther, int& pNbHitHimself) const
		{
		...
		/*
		BOOL CentralisedNetworkSession::GetResult( int pPosition, const char*& pPlayer, MR_SimulationTime& pFinishTime, MR_SimulationTime& pBestLap )const
		{
		...
		BOOL CentralisedNetworkSession::ResultAvaillable()const
		{
		...
		*/
		 ...
		 */
		int CentralisedNetworkSession::GetNbPlayers() const
		{
		 ...
		 */
		MainCharacter::MainCharacter* CentralisedNetworkSession::GetPlayer(int pPlayerIndex) const
		{
		 ...
		 */
		int CentralisedNetworkSession::GetRank(const MainCharacter::MainCharacter* /*pPlayer */) const {
		 ...
		 */
		void CentralisedNetworkSession::Process(int pSpeedFactor)
		{
			ReadNet();
		 ...
		 */
		BOOL CentralisedNetworkSession::LoadNew(const char* pTitle, HoverRace::Parcel::RecordFilePtr pMazeFile, int pNbLap, char pGameOpts, VideoServices::VideoBuffer* pVideo)
		{
		 ...
		 */
		void CentralisedNetworkSession::ElementCreationHook(Model::FreeElement* pElement, int pRoom, void* pThis)
		{
			((CentralisedNetworkSession*)pThis)->BroadcastAutoElementCreation(pElement->GetTypeId(), pElement->GetNetState(), pRoom);
		}

		/**
		 * A hook called when a permanent element is created.  We need to broadcast it once it is created.
		 */
		void CentralisedNetworkSession::PermElementStateHook(Model::FreeElement* pElement, int pRoom, int pPermId, void* pThis)
		{
			((CentralisedNetworkSession*)pThis)->BroadcastPermElementState(pPermId, pElement->GetNetState(), pRoom);
		}
		 ...
		 */
		void CentralisedNetworkSession::ReadNet()
		{
			DWORD lTimeStamp;
			int lMessageType;
			int lMessageLen;
		...
		/**
		 * Send all necessary data to clients.  This is called by CentralisedNetworkSession::Process() as part of the game loop.
		 *
		 ...
		 */
		void CentralisedNetworkSession::WriteNet()
		{
		 ...
		 */
		void CentralisedNetworkSession::SetPlayerName(const char* pPlayerName)
		{
		 ...
		 */
		const char* CentralisedNetworkSession::GetPlayerName() const
		{
			return mNetInterface.GetPlayerName();
		}

		void CentralisedNetworkSession::SetRoomList(RoomListPtr roomList)
		{
		 ...
		 */
		BOOL CentralisedNetworkSession::WaitConnections(HWND pWindow, const char* pTrackName, BOOL pPromptForPort, unsigned pDefaultPort, HWND* pModalessDlg, int pReturnMessage)
		{
			mMasterMode = TRUE;
		 ...
		 */
		BOOL CentralisedNetworkSession::PreConnectToServer(HWND pWindow, std::string& pTrackName)
		{
			mMasterMode = FALSE;
		 ...
		 * @param pReturnMessage Callback message (see NetworkInterface::ListCallBack);
		 */
		BOOL CentralisedNetworkSession::ConnectToServer(HWND pWindow, const char* pServerIP, unsigned pPort, const char* pGameName, HWND* pModalessDlg, int pReturnMessage)
		{
			mMasterMode = FALSE;
		 ...
		 */
		void CentralisedNetworkSession::SetSimulationTime(MR_SimulationTime pTime)
		{
		 ...
		 */
		BOOL CentralisedNetworkSession::CreateMainCharacter()
		{
		 ...
		 */
		void CentralisedNetworkSession::BroadcastMainElementCreation(const Util::ObjectFromFactoryId& pId, const Model::ElementNetState& pState, int pRoom, int pHoverId)
		{
		 ...
		 */
		void CentralisedNetworkSession::BroadcastAutoElementCreation(const Util::ObjectFromFactoryId& pId, const Model::ElementNetState& pState, int pRoom)
		{
			CentralisedNetMessageBuffer lMessage;
			int lCounter;
		 ...
		 */
		void CentralisedNetworkSession::BroadcastPermElementState(int pPermId, const Model::ElementNetState& pState, int pRoom)
		{
			CentralisedNetMessageBuffer lMessage;
			int lCounter;
		 ...
		 */
		void CentralisedNetworkSession::BroadcastTime()
		{
			CentralisedNetMessageBuffer lMessage;
		 ...
		 */
		void CentralisedNetworkSession::BroadcastMainElementState(const Model::ElementNetState& pState)
		{
			CentralisedNetMessageBuffer lMessage;
			...
			if (lMaxSend > 0) {
				...
				for (int lNbToSend = min(lNbEligible, lMaxSend); lNbToSend > 0; lNbToSend--) {
					// Find the high?8est priority element
		 ...
		 */
		void CentralisedNetworkSession::BroadcastMainElementStats(MR_SimulationTime pFinishTime, MR_SimulationTime pBestLap, int pNbLaps)
		{
			CentralisedNetMessageBuffer lMessage;
		 ...
		 */
		void CentralisedNetworkSession::BroadcastChatMessage(const char* pMessage)
		{
			CentralisedNetMessageBuffer lMessage;
		 ...
		 */
		void CentralisedNetworkSession::BroadcastHit(int pHoverIdSrc)
		{
			CentralisedNetMessageBuffer lMessage;
		 ...
		 */
		void CentralisedNetworkSession::AddHitEntry(int pPlayerIndex, int pPlayerFromId)
		{
		 ...
		 */
		void CentralisedNetworkSession::InsertHitEntry(PlayerResult* pEntry)
		{
			PlayerResult** lPtr;
		 ...
		 */
		void CentralisedNetworkSession::AddResultEntry(int pPlayerIndex, MR_SimulationTime pFinishTime, MR_SimulationTime pBestLap, int pNbLap)
		{
		 ...
		 */
		void CentralisedNetworkSession::AddMessageKey(char pKey)
		{
			//	boost::lock_guard<boost::mutex> lock(chatMutex);
		 ...
		 */
		void CentralisedNetworkSession::GetCurrentMessage(char* pDest) const
		{
			boost::lock_guard<boost::mutex> lock(chatMutex);
		 ...
		 */
		void CentralisedNetworkSession::AddChatMessage(int pPlayerIndex, const char* pMessage, int pMessageLen)
		{
