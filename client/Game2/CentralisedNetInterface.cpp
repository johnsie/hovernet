...
//Adding netcommon so we can handle the 
...
namespace HoverRace {
	namespace Client {
		...
		CentralisedNetworkInterface* CentralisedNetworkInterface::mActiveInterface = NULL;

		/**
		 * Set up the CentralisedNetworkInterface.  Set up the client list and information, create and initialize both UDPOut ports, set them as non-blocking, and
		 * set the player name (mPlayer) correctly.
		 */
		CentralisedNetworkInterface::CentralisedNetworkInterface()
		{
		...
		/**
		 * Destroy the CentralisedNetworkInterface.  Close the UDP out sockets, disconnect, and call WSACleanup() to clean up WSA-related cruft.
		 */
		CentralisedNetworkInterface::~CentralisedNetworkInterface()
		{
		 ...
		 */
		const char* CentralisedNetworkInterface::GetPlayerName(int pIndex) const
		{
		 ...
		 */
		BOOL CentralisedNetworkInterface::IsConnected(int pIndex) const
		{
		 ...
		 */
		void CentralisedNetworkInterface::Disconnect()
		{
		 ...
		 */
		int CentralisedNetworkInterface::GetClientCount() const
		{
			int lReturnValue = 0;
		 ...
		 */
		int CentralisedNetworkInterface::GetId() const
		{
		 ...
		 */
		int CentralisedNetworkInterface::GetLagFromServer() const
		{
			if (mServerMode)
				return 0;
			else
		 ...
		 */
		BOOL CentralisedNetworkInterface::CreateUDPRecvSocket(int pPort)
		{
		 ...
		 */
		int CentralisedNetworkInterface::GetMinLag(int pClient) const
		{
		 ...
		 * @param pClient Index of the client
		 */
		int CentralisedNetworkInterface::GetAvgLag(int pClient) const
		{
			ASSERT((pClient >= 0) && (pClient < eMaxClient));
			return mClient[pClient].GetMinLag();
		}
		 ...
		 */
		BOOL CentralisedNetworkInterface::UDPSend(int pClient, CentralisedNetMessageBuffer* pMessage, BOOL pLongPort, BOOL pResendLast)
		{
			ASSERT((pClient >= 0) && (pClient < eMaxClient));
		 ...
		 */
		BOOL CentralisedNetworkInterface::BroadcastMessage(CentralisedNetMessageBuffer* pMessage, int pReqLevel)
		{
		...
		/*
		BOOL CentralisedNetworkInterface::BroadcastMessage( DWORD  pTimeStamp, int  pMessageType, int pMessageLen, const MR_UInt8* pMessage )
		{
		...
		*/
		 ...
		 */
		BOOL CentralisedNetworkInterface::FetchMessage(DWORD& pTimeStamp, int& pMessageType, int& pMessageLen, const MR_UInt8*& pMessage, int& pClientId)
		{
			...
			for (int lCounter = 0; !lReturnValue && (lCounter < eMaxClient); lCounter++) {
				...
				const CentralisedNetMessageBuffer* lMessage = mClient[lClient].Poll((lClient >= mId) ? lClient + 1 : lClient, TRUE);
		 ...
		 */
		void CentralisedNetworkInterface::SetPlayerName(const char* pPlayerName)
		{
			mPlayer = pPlayerName;
		}
		 ...
		 * Get the player name.
		 */
		const char* CentralisedNetworkInterface::GetPlayerName() const
		{
		 ...
		 */
		BOOL CentralisedNetworkInterface::MasterConnect(HWND pWindow, const char* pGameName, BOOL pPromptForPort, unsigned pDefaultPort, HWND* pModalessDlg, int pReturnMessage)
		{
			BOOL lReturnValue = FALSE;
		 ...
		 * selected.  When the dialog box is filled out, CentralisedNetworkInterface::ServerAddrCallBack is called.
		 *
		 * @param pWindow Parent window
		 * @param pGameName String to store the name of the game into
		 */
		BOOL CentralisedNetworkInterface::SlavePreConnect(HWND pWindow, std::string& pGameName)
		{
			BOOL lReturnValue = FALSE;
			mActiveInterface = this;
		 ...
		 * phase never happened) which calls CentralisedNetworkInterface::ServerAddrCallBack() and later a dialog (the "TCP Connections" waiting screen) that calls
		 * CentralisedNetworkInterface::ListCallBack().
		 *
		 * @param pWindow Parent window
		 ...
		 * @param pReturnMessage Callback message (see ListCallBack())
		 */
		BOOL CentralisedNetworkInterface::SlaveConnect(HWND pWindow, const char* pServerIP, unsigned pDefaultPort, const char* pGameName, HWND* pModalessDlg, int pReturnMessage)
		{
			ASSERT(!mServerMode);
		 ...
		 */
		BOOL CALLBACK CentralisedNetworkInterface::ServerPortCallBack(HWND pWindow, UINT pMsgId, WPARAM pWParam, LPARAM pLParam)
		{
			BOOL lReturnValue = FALSE;
			...
			case WM_INITDIALOG: // set up the dialog
				...
				SetDlgItemText(pWindow, IDC_SERVER_ADDR, "127.0.0.1");
		 ...
		 * @param pLParam not used but required for MFC callback
		 */
		BOOL CALLBACK CentralisedNetworkInterface::ServerAddrCallBack(HWND pWindow, UINT pMsgId, WPARAM pWParam, LPARAM pLParam)
		{
			BOOL lReturnValue = FALSE;
			...
			case WM_INITDIALOG: // set up the dialog
				SetDlgItemText(pWindow, IDC_PLAYER_NAME, mActiveInterface->mPlayer.c_str());
				SetDlgItemText(pWindow, IDC_SERVER_ADDR, "127.0.0.1");
				SetDlgItemInt(pWindow, IDC_SERVER_PORT, 953000, FALSE); // 9530 is the default port
				lReturnValue = TRUE;
				break;
			...
			case WM_COMMAND: // command was given
				...
				case IDC_DEFAULT: // set the default port
					SetDlgItemInt(pWindow, IDC_SERVER_PORT, 953000, FALSE);
					lReturnValue = TRUE;
					break;
				...
				case IDOK:
				{
					...
					//user has clicked ok to join a game. This is where we get the infor and connect to the server
					// retrieve player name
					char lNameBuffer[80];
					GetDlgItemText(pWindow, IDC_PLAYER_NAME, lNameBuffer, sizeof(lNameBuffer));
					mActiveInterface->mPlayer = lNameBuffer;
		 ...
		 * @param pLParam not used but required for MFC callback
		 */
		BOOL CALLBACK CentralisedNetworkInterface::WaitGameNameCallBack(HWND pWindow, UINT pMsgId, WPARAM pWParam, LPARAM pLParam)
		{
			BOOL lReturnValue = FALSE;
			...
			{
				CentralisedNetMessageBuffer lOutputBuffer;
			...
			case MRM_CLIENT:
			{
				const CentralisedNetMessageBuffer* lBuffer = NULL;
		 ...
		 * @param pLParam not used but required for MFC callback
		 */
		BOOL CALLBACK CentralisedNetworkInterface::ListCallBack(HWND pWindow, UINT pMsgId, WPARAM pWParam, LPARAM pLParam)
		{
			BOOL lReturnValue = FALSE;
			...
			case WM_COMMAND:
				...
				case IDOK:
				{
					...
					if (lOk) {
						...
						if (lOk) {
							CentralisedNetMessageBuffer lMessage;
		 ...
		 */
		BOOL CALLBACK CentralisedNetworkInterface::WarnAloneCallback(HWND pWindow, UINT pMsgId, WPARAM pWParam, LPARAM pLParam) {
			switch (pMsgId) {
			case WM_INITDIALOG: // set up the window
				// do i18n
		 ...
		 * It is rather simple.
		 */
		BOOL CALLBACK CentralisedNetworkInterface::WaitConnCallback(HWND pWindow, UINT pMsgId, WPARAM pWParam, LPARAM pLParam) {
			switch (pMsgId) {
			case WM_INITDIALOG: // set up the window
				// do i18n
				SetWindowTextW(pWindow, Str::UW(_("Warning!")));
		 ...
		 */
		void CentralisedNetworkInterface::SendConnectionDoneIfNeeded()
		{
			CentralisedNetMessageBuffer lAnswer;
		 ...
		 */
		CentralisedNetworkPort::CentralisedNetworkPort()
		{
		 ...
		 */
		void CentralisedNetworkPort::Connect(SOCKET pSocket, SOCKET pUDPRecvSocket)
		{
			Disconnect();
			mSocket = pSocket;
		 ...
		 */
		void CentralisedNetworkPort::SetRemoteUDPPort(unsigned int pPort)
		{
		 ...
		 */
		void CentralisedNetworkPort::Disconnect()
		{
		 ...
		 */
		BOOL CentralisedNetworkPort::IsConnected() const
		{
		 ...
		 */
		SOCKET CentralisedNetworkPort::GetSocket() const
		{
			return mSocket;
		}
		 ...
		 */
		SOCKET CentralisedNetworkPort::GetUDPSocket() const
		{
		 ...
		 */
		const CentralisedNetMessageBuffer* CentralisedNetworkPort::Poll(int pClientId, BOOL pCheckClientId)
		{
		 ...
		 *
		 * @param pSocket The socket the packet will be sent from (generally CentralisedNetworkInterface::mUDPOutLongPort or CentralisedNetworkInterface::mUDPOutShortPort)
		 ...
		 */
		BOOL CentralisedNetworkPort::UDPSend(SOCKET pSocket, CentralisedNetMessageBuffer* pMessage, unsigned pQueueId, BOOL pResendLast)
		{
			BOOL lReturnValue = TRUE;
		 ...
		 */
		void CentralisedNetworkPort::Send(const CentralisedNetMessageBuffer* pMessage, int pReqLevel)
		{
		 ...
		 */
		BOOL CentralisedNetworkPort::AddLagSample(int pLag)
		{
			mNbLagTest++;
		 ...
		 */
		BOOL CentralisedNetworkPort::LagDone() const
		{
			return (mNbLagTest >= 5);
		}
		 ...
		 */
		int CentralisedNetworkPort::GetAvgLag() const
		{
			return mAvgLag;
		}
		 ...
		 */
		int CentralisedNetworkPort::GetMinLag() const
		{
			return mMinLag;
		}
		 ...
		 */
		void CentralisedNetworkPort::SetLag(int pAvgLag, int pMinLag)
		{
			mAvgLag = pAvgLag;
			mMinLag = pMinLag;
