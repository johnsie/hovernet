...
#define MR_NET_HEADER_LEN  (sizeof(CentralisedNetMessageBuffer) - MR_MAX_NET_MESSAGE_LEN)
		...
		public:
			...
			const CentralisedNetMessageBuffer* Poll(int pClientId, BOOL pCheckClientId); // parameters are hacks
			void Send(const CentralisedNetMessageBuffer* pMessage, int pReqLevel);
			BOOL UDPSend(SOCKET pSocket, CentralisedNetMessageBuffer* pMessage, unsigned pQueueId, BOOL pResendLast);
		 ...
		 */
		class CentralisedNetworkInterface
		{
		public:
			enum {
				eMaxClient = 9
		...
		private:
			...
			// Data
			CentralisedNetworkPort mClient[eMaxClient];
			...
			// Dialog functions
			static CentralisedNetworkInterface* mActiveInterface;
		...
		public:
			...
			BOOL UDPSend(int pClient, CentralisedNetMessageBuffer* pMessage, BOOL pLongPort, BOOL pResendLast = FALSE);
			BOOL BroadcastMessage(CentralisedNetMessageBuffer* pMessage, int pReqLevel);
