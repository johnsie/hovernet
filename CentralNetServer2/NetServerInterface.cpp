...
namespace HoverRace {
	namespace CentralNetServer2 {
		...
		NetServerInterface* NetServerInterface::mActiveInterface = NULL;

		/**
		 * Set up the NetServerInterface.  Set up the client list and information, create and initialize both UDPOut ports, set them as non-blocking, and
		...
		/**
		 * Destroy the NetServerInterface.  Close the UDP out sockets, disconnect, and call WSACleanup() to clean up WSA-related cruft.
		 */
		NetServerInterface::~NetServerInterface()
		{
		 ...
		 */
		const char* NetServerInterface::GetPlayerName(int pIndex) const
		{
		 ...
		 */
		BOOL NetServerInterface::IsConnected(int pIndex) const
		{
		 ...
		 */
		void NetServerInterface::Disconnect()
		{
		 ...
		 */
		int NetServerInterface::GetClientCount() const
		{
			int lReturnValue = 0;
		 ...
		 */
		int NetServerInterface::GetId() const
		{
		 ...
		 */
		int NetServerInterface::GetLagFromServer() const
		{
			if (mServerMode)
				return 0;
			else
		 ...
		 */
		BOOL NetServerInterface::CreateUDPRecvSocket(int pPort)
		{
		 ...
		 */
		int NetServerInterface::GetMinLag(int pClient) const
		{
		 ...
		 * @param pClient Index of the client
		 */
		int NetServerInterface::GetAvgLag(int pClient) const
		{
			ASSERT((pClient >= 0) && (pClient < eMaxClient));
			return mClient[pClient].GetMinLag();
		}
		 ...
		 */
		BOOL NetServerInterface::UDPSend(int pClient, CentralisedNetMessageBuffer* pMessage, BOOL pLongPort, BOOL pResendLast)
		{
			ASSERT((pClient >= 0) && (pClient < eMaxClient));
		 ...
		 */
		BOOL NetServerInterface::BroadcastMessage(CentralisedNetMessageBuffer* pMessage, int pReqLevel)
		{
		...
		/*
		BOOL NetServerInterface::BroadcastMessage( DWORD  pTimeStamp, int  pMessageType, int pMessageLen, const MR_UInt8* pMessage )
		{
		...
		*/
		 ...
		 */
		BOOL NetServerInterface::FetchMessage(DWORD& pTimeStamp, int& pMessageType, int& pMessageLen, const MR_UInt8*& pMessage, int& pClientId)
		{
		 ...
		 */
		void NetServerInterface::SetPlayerName(const char* pPlayerName)
		{
			mPlayer = pPlayerName;
		}
		 ...
		 * Get the player name.
		 */
		const char* NetServerInterface::GetPlayerName() const
		{
		 ...
		 */
		BOOL NetServerInterface::MasterConnect(HWND pWindow, const char* pGameName, BOOL pPromptForPort, unsigned pDefaultPort, HWND* pModalessDlg, int pReturnMessage)
		{
			BOOL lReturnValue = FALSE;
		 ...
		 * selected.  When the dialog box is filled out, NetServerInterface::ServerAddrCallBack is called.
		 *
		 * @param pWindow Parent window
		 * @param pGameName String to store the name of the game into
		 */
		BOOL NetServerInterface::SlavePreConnect(HWND pWindow, std::string& pGameName)
		{
			BOOL lReturnValue = FALSE;
			mActiveInterface = this;
		 ...
		 * phase never happened) which calls NetServerInterface::ServerAddrCallBack() and later a dialog (the "TCP Connections" waiting screen) that calls
		 * NetServerInterface::ListCallBack().
		 *
		 * @param pWindow Parent window
		 ...
		 * @param pReturnMessage Callback message (see ListCallBack())
		 */
		BOOL NetServerInterface::SlaveConnect(HWND pWindow, const char* pServerIP, unsigned pDefaultPort, const char* pGameName, HWND* pModalessDlg, int pReturnMessage)
		{
			ASSERT(!mServerMode);
		 ...
		 */
		BOOL CALLBACK NetServerInterface::ServerPortCallBack(HWND pWindow, UINT pMsgId, WPARAM pWParam, LPARAM pLParam)
		{
			BOOL lReturnValue = FALSE;
		 ...
		 * @param pLParam not used but required for MFC callback
		 */
		BOOL CALLBACK NetServerInterface::ServerAddrCallBack(HWND pWindow, UINT pMsgId, WPARAM pWParam, LPARAM pLParam)
		{
			BOOL lReturnValue = FALSE;
		 ...
		 * @param pLParam not used but required for MFC callback
		 */
		BOOL CALLBACK NetServerInterface::WaitGameNameCallBack(HWND pWindow, UINT pMsgId, WPARAM pWParam, LPARAM pLParam)
		{
			BOOL lReturnValue = FALSE;
		 ...
		 * @param pLParam not used but required for MFC callback
		 */
		BOOL CALLBACK NetServerInterface::ListCallBack(HWND pWindow, UINT pMsgId, WPARAM pWParam, LPARAM pLParam)
		{
			BOOL lReturnValue = FALSE;
		 ...
		 */
		BOOL CALLBACK NetServerInterface::WarnAloneCallback(HWND pWindow, UINT pMsgId, WPARAM pWParam, LPARAM pLParam) {
			switch (pMsgId) {
			case WM_INITDIALOG: // set up the window
				// do i18n
		 ...
		 * It is rather simple.
		 */
		BOOL CALLBACK NetServerInterface::WaitConnCallback(HWND pWindow, UINT pMsgId, WPARAM pWParam, LPARAM pLParam) {
			switch (pMsgId) {
			case WM_INITDIALOG: // set up the window
				// do i18n
				SetWindowTextW(pWindow, Str::UW(_("Warning!")));
		 ...
		 */
		void NetServerInterface::SendConnectionDoneIfNeeded()
		{
			CentralisedNetMessageBuffer lAnswer;
		 ...
		 *
		 * @param pSocket The socket the packet will be sent from (generally NetServerInterface::mUDPOutLongPort or NetServerInterface::mUDPOutShortPort)
